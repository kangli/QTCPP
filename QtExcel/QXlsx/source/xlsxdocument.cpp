	...
	d_ptr(new DocumentPrivate(this))
{
	...
	{
		QFile xlsx(name);
        bool f=xlsx.open(QFile::ReadOnly,QFileDevice::ReadOther);
        if (f)
		{
...
{
    QFile file(name);
    qDebug()<< "文件是否已经存在"<<file.exists();
    bool f=file.open(QIODevice::WriteOnly,QFileDevice::WriteOther);
    qDebug()<< "文件是否已经打开"<< file.isOpen();
    if (f){
        return saveAs(&file);
    }
